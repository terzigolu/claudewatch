...
3. Configure the build (Cloudflare's unified UI detects `wrangler.toml` at repo root):
   ...
   `pages_build_output_dir = "./site/dist"`. If `ccwatch.statusline.workers.dev` is
...
5. Site is live at `https://ccwatch.statusline.workers.dev/` (or your chosen subdomain).
...
- Add CNAME record pointing to `ccwatch.statusline.workers.dev`
- HTTPS is automatic
